...
|规则|触发和边界|具体动作|视觉影响|
|---|---|---|---|
|R01 减层|明确、可信的非必要装饰组件；不删除保护/密封/屏障/关键品牌件；有保留的保护组件|对命名组件 `remove`|high|
|R02 缩容|有可用空间利用率与外盒目标；仅泛化 medium 先验不足以触发|外盒 `resize`，内托 `resize_to_fit`|high|
...
商业分以82为工程先验基线，读取按“材料质量×相对成本指数”得到的预计材料成本变化，并计入减工序、缩容收益；再扣除工艺、品牌、开箱体验、新模具、新供应商、材料替换及动作复杂度风险。供应链分以96为保守的现状可实施性基线，扣除材料可得性、工艺、运输保护、新模具、新供应商和动作复杂度风险。两者均限制在0–100。
...
1. `carbon_data` 材料参考路径：确认材料声明及逐材料质量；完整 BOM 且所有因子可用才给总量。
2. `carbon_data.visual_estimate`：采用估算质量；单一材料或受支持的纸+塑料质量分配才能计算，不重复把整包重量计入每种材料。

数字模型路径会为可映射的每个 Before/After 组件重新计算材料质量与参考碳排。DEFRA 因子进入 `carbon_reduction_score`，从而直接影响环境分；未知材料、缺因子或不完整组件仍不会被当作零排放。这些结果不涵盖全部制造、运输、使用与终端处理生命周期。
...
最近一次本地代码回归记录：111项测试通过，2条第三方弃用警告；前端任务轮询与内联 JavaScript 语法检查通过。没有为测试调用付费模型。
